module Surveyable
  module SurveysHelper
    ...
    def link_to_add_fields(name, form, association)
      ...
      4.times { new_object.answers.build } if new_object.class.to_s == 'Surveyable::Question'

      fields = form.fields_for(association, new_object, child_index: "new_#{association}") do |builder|
        render(association.to_s.singularize + "_fields", form: builder)
      end

      link_to_function(name, "add_fields(this, \"#{association}\", \"#{escape_javascript(fields)}\")", class: 'add-answers-link button icon-plus')
    end
  end
end
